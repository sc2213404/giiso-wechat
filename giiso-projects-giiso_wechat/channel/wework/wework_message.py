...
def cdn_download(wework, message, save_path):
    data = message["data"]
...
def c2c_download_and_convert(wework, message, save_path):
    data = message["data"]
    aes_key = data["cdn"]["aes_key"]
    file_size = data["cdn"]["size"]
    file_type = 5
...
class WeworkMessage(ChatMessage):
    def __init__(self, wework_msg, wework, channel: Channel, is_group=False):
        try:
            ...
            if wework_msg["type"] == 11041:  # 文本消息类型
                if any(substring in wework_msg['data']['content'] for substring in
                       ("该消息类型暂不能展示", "不支持的消息类型")):
                    return
                ...
                self.content = os.path.join(current_dir, "output", "voice", self.channel.user_id, wework_msg['data'].get("sender"), file_name_2)
                silk_file_path = os.path.join(current_dir, "output", "voice", self.channel.user_id, wework_msg['data'].get("sender"), file_name)
                self._prepare_fn = lambda: c2c_download_and_convert(wework, wework_msg, silk_file_path)
            elif wework_msg["type"] == 11042:  # 图片消息类型，需要下载文件
                file_name = datetime.datetime.now().strftime('%Y%m%d%H%M%S') + ".jpg"
                current_dir = os.getcwd()
                self.ctype = ContextType.IMAGE
                self.content = os.path.join(current_dir, "output", "picture", self.channel.user_id,  wework_msg['data'].get("sender"), file_name)
                self._prepare_fn = lambda: cdn_download(wework, wework_msg, self.content)
                ...
                self.content = os.path.join(current_dir, "output", "file", self.channel.user_id,  wework_msg['data'].get("sender"), file_name)
                self._prepare_fn = lambda: cdn_download(wework, wework_msg, self.content)
                ...
                directory = os.path.join(os.getcwd(), "output", "json", self.channel.user_id)
            ...
            nickname = f"{login_info['username']}({login_info['nickname']})" if login_info['nickname'] else login_info[
                'username']
